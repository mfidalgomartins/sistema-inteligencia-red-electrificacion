"""Pronóstico de demanda por zona con error NMAE comparable entre zonas."""
...
from .forecast_validation import run_continuous_forecast_validation
...
def run_forecasting() -> dict[str, pd.DataFrame]:
    ...
    best_model_dz = benchmark[benchmark["task"] == "demanda_zona"].sort_values("mae").iloc[0]["model"]
    ...
    # Error en horas punta: pronóstico horario de demanda por zona.
    ...
    predictability = dz_best.groupby("zona_id", as_index=False).agg(
        mae=("abs_error", "mean"),
    ...
    predictability["ratio_industrial"] = predictability["presion_industrial"] / predictability["demanda_media"].replace(
        0, np.nan
    )
    predictability["ratio_nueva_demanda"] = (
        predictability["presion_ev"] + predictability["presion_industrial"]
    ) / predictability["demanda_media"].replace(0, np.nan)
    ...
    continuous_validation = run_continuous_forecast_validation(
        zone_day,
        models,
        _predict_one_step,
        paths,
