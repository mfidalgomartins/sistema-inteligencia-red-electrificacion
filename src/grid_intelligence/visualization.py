"""Gráficos exploratorios del pipeline (matplotlib), previos a la publicación."""
...
plot_cache_dir = Path(gettempdir()) / "red-electrificacion-cache-graficos"
...
def run_visualization() -> list[str]:
    ...
    c1 = (
        node.assign(month=pd.to_datetime(node["timestamp"]).dt.to_period("M").astype(str))
        .groupby("month", as_index=False)["carga_relativa"]
        .mean()
    )
    ...
    # 3) Mapa de calor de congestión por hora y territorio.
    zone_hour = (
    ...
    ax.set_title("Mapa de calor de congestión por hora y zona")
    ...
    # 4) Subestaciones líderes por exposición.
    c4 = (
    ...
    ax.set_title("Subestaciones líderes por exposición a congestión")
    ...
    # 5) Alimentadores líderes por criticidad.
    c5 = (
        node.groupby("alimentador_id", as_index=False)
        .agg(
            criticidad_media=("criticidad_operativa", "mean"),
    ...
    ax.set_title("Alimentadores líderes por criticidad operativa")
    ...
    ax.scatter(
        flex_gap["cobertura_flexible_total_mw"],
        flex_gap["demanda_critica_mw"],
        c=flex_gap["riesgo_operativo_score"],
        cmap="viridis",
        s=70,
    )
    ...
    ax.set_title("Capacidad de almacenamiento frente a estrés operativo")
    ax.set_xlabel("Potencia total de almacenamiento (MW)")
    ax.set_ylabel("Puntuación de riesgo operativo")
    fig.tight_layout()
    ...
    # 9) Impacto de vehículos eléctricos sobre carga crítica.
    c9 = zone_day.groupby("zona_id", as_index=False).agg(
        demanda_ev_total=("demanda_ev_total", "sum"), percentil_carga=("percentil_carga", "mean")
    )
    fig, ax = plt.subplots(figsize=(10, 6))
    ...
    ax.set_title("Impacto de vehículos eléctricos sobre carga crítica")
    ax.set_xlabel("Demanda total de vehículos eléctricos")
    ...
    c10 = zone_day.groupby("zona_id", as_index=False).agg(
        demanda_industrial=("demanda_industrial_adicional_total", "sum"), horas_congestion=("horas_congestion", "sum")
    )
    fig, ax = plt.subplots(figsize=(10, 6))
    ...
    # 11) Vertido por territorio.
    c11 = (
        zone_day.groupby("zona_id", as_index=False)["curtailment_total"]
        .sum()
        .sort_values("curtailment_total", ascending=False)
    )
    fig, ax = plt.subplots(figsize=(12, 5))
    ax.bar(c11["zona_id"], c11["curtailment_total"], color="#fb9a99")
    ax.set_title("Vertido acumulado por territorio")
    ax.set_xlabel("Zona")
    ax.set_ylabel("Vertido total")
    ax.tick_params(axis="x", rotation=60)
    fig.tight_layout()
    ...
    ax.set_xlabel("Puntuación de riesgo de congestión")
    ax.set_ylabel("Puntuación de prioridad económica")
    fig.tight_layout()
    ...
    ax.set_xlabel("Zona")
    ax.set_ylabel("Puntuación de prioridad de inversión")
    ax.tick_params(axis="x", rotation=60)
    fig.tight_layout()
    ...
    ax.plot(
        ...
        scenario_summary["coste_riesgo_total"],
        marker="o",
        linewidth=2.0,
        label="Coste riesgo",
    )
    ax.plot(
        scenario_summary["scenario"],
        scenario_summary["inversion_requerida_total"],
        marker="s",
        linewidth=2.0,
        label="Inversión",
    )
    ...
    ax.set_ylabel("EUR de referencia")
    ...
    # 15) Factores de puntuación por zona.
    ...
    for d, c in zip(drivers, colors, strict=False):
        ...
        bottom += vals
    ax.set_title("Factores de puntuación por zona (prioridades líderes)")
    ax.set_xlabel("Zona")
    ...
    explanations = [
        ...
        "3. Mapa de calor hora-territorio: patrón intradía de congestión.",
        "4. Subestaciones líderes por exposición: focos críticos de operación.",
        "5. Alimentadores líderes por criticidad: activos lineales con mayor riesgo.",
        ...
        "9. Impacto de vehículos eléctricos sobre carga crítica: presión de nueva demanda frente a percentil de carga.",
        "10. Impacto industrial: electrificación industrial frente a congestión.",
        "11. Vertido por territorio: zonas con mayor recorte de GD.",
        ...
        "15. Factores de puntuación por zona: composición del riesgo agregado.",
    ]
