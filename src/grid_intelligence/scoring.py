"""Scoring multicriterio de zonas: riesgo, servicio, flexibilidad, activos y economía."""
...
def _apply_intervention_rules(base: pd.DataFrame) -> pd.DataFrame:
    ...
    out.loc[
        (out["congestion_risk_score"] >= 80) & (out["ratio_flexibilidad_estres"] < 0.15),
        "recommended_intervention",
    ] = "reforzar_red_local"
    out.loc[
        (out["flexibility_gap_score"] >= 80) & (out["service_impact_score"] >= 65) & (out["storage_efectivo"] < 0.03),
        "recommended_intervention",
...
def run_scoring() -> dict[str, pd.DataFrame]:
    ...
    anomaly = (
        pd.read_csv(paths.data_processed / "anomaly_zone_intensity.csv")
        if (paths.data_processed / "anomaly_zone_intensity.csv").exists()
        else pd.DataFrame()
    )
    forecast = (
    ...
    if not forecast.empty:
        base = base.merge(
            forecast[["zona_id", "mae", "nmae", "ratio_nueva_demanda", "decision_forecast"]], on="zona_id", how="left"
        )
    else:
        base["mae"] = 0.0
        base["nmae"] = 0.0
    ...
    base["congestion_risk_score"] = (
        ...
        * minmax(
            base[
                "recurrencia_congestion" if "recurrencia_congestion" in base.columns else "horas_congestion_acumuladas"
            ]
        )
    ...
    for _, row in base.iterrows():
        options = {
            "refuerzo_red": {
                "impact": 0.55 * row["congestion_risk_score"]
                + 0.25 * row["electrification_pressure_score"]
                + 0.20 * row["service_impact_score"],
                "cost": row["capex_total"],
                "time": row["horizonte_medio"],
                "robustez": 0.90,
            },
            "flexibilidad": {
                "impact": 0.45 * row["flexibility_gap_score"]
                + 0.25 * row["congestion_risk_score"]
                + 0.30 * row["economic_priority_score"],
                "cost": 0.35 * row["capex_total"],
                "time": 0.55 * row["horizonte_medio"],
                "robustez": 0.68,
            },
            "almacenamiento": {
                "impact": 0.40 * row["flexibility_gap_score"]
                + 0.35 * row["service_impact_score"]
                + 0.25 * row["electrification_pressure_score"],
            ...
            "intervencion_operativa": {
                "impact": 0.35 * row["congestion_risk_score"]
                + 0.30 * row["asset_exposure_score"]
                + 0.35 * row["resilience_risk_score"],
    ...
    best_option = (
        options_df.sort_values(["zona_id", "option_score"], ascending=[True, False])
    ...
    base["investment_priority_score"] = (0.74 * urgency_base + 0.26 * base["option_score"]).clip(0, 100)
    ...
    # Sensibilidad de puntuación: perturbación simple de pesos +/-10%.
    sens = []
    ...
    # Persistencia canónica en DuckDB para consumo de validación/tablero.
    ...
    conn.execute(
        "CREATE OR REPLACE TABLE intervention_multicriteria_options AS SELECT * FROM tmp_intervention_multicriteria_options"
    )
    conn.execute(
        "CREATE OR REPLACE TABLE scoring_sensitivity_analysis AS SELECT * FROM tmp_scoring_sensitivity_analysis"
    )
    ...
    framework_doc = (
        ...
        ## Principio
        Puntuación lineal interpretable que combina criterios técnicos, de servicio, activos, electrificación y economía.

        ## Fórmulas (resumen)
        - Cada puntuación parcial se construye con combinación lineal ponderada de señales normalizadas (0-100).
        ...
        - Almacenamiento equilibra impacto técnico y resiliencia en zonas con alta variabilidad.
        ...
        - brecha de flexibilidad `>= 80`, impacto de servicio `>= 65` y almacenamiento efectivo `< 0.03` fuerza almacenamiento.
        ...
        ## Escenarios
        Los factores de escenario se localizan por zona según los factores de puntuación relevantes.
        La validación bloquea la publicación si todos los escenarios producen el mismo ranking.
