"""Motor de escenarios: recalcula riesgo e inversión bajo supuestos alternativos."""
...
def run_scenario_engine() -> dict[str, pd.DataFrame]:
    ...
    for scenario, params in SCENARIOS.items():
        ...
        sdf["carga_punta_scenario_mw"] = sdf["carga_punta_base"] * _localized_factor(
            params["load_factor"], sdf["scenario_exposure"]
        )
        sdf["horas_congestion_scenario"] = sdf["horas_congestion_avg"] * _localized_factor(
            params["congestion_factor"], sdf["scenario_exposure"]
        )
        sdf["ens_scenario"] = sdf["ens_avg"] * _localized_factor(params["ens_factor"], sdf["scenario_exposure"])
        sdf["curtailment_scenario"] = sdf["curtailment_base"] * _localized_factor(
            params["curtailment_factor"], sdf["scenario_exposure"]
        )
        sdf["flexibility_gap_scenario"] = sdf["gap_flexibilidad"] * _localized_factor(
            params["flex_gap_factor"], sdf["scenario_exposure"]
        )
        sdf["coste_riesgo_scenario"] = sdf["coste_riesgo_proxy"] * _localized_factor(
            params["risk_cost_factor"], sdf["scenario_exposure"]
        )
        sdf["inversion_requerida_scenario"] = sdf["capex_total"] * _localized_factor(
            params["capex_factor"], sdf["scenario_exposure"]
        ) + 0.20 * sdf["intensidad_capex_base"] * _localized_factor(
            max(params["flex_gap_factor"], 1.0), sdf["scenario_exposure"]
        )
        sdf["investment_priority_score_scenario"] = (
            sdf["investment_priority_score"] * _localized_factor(params["priority_factor"], sdf["scenario_exposure"])
        ).clip(0, 100)
    ...
    priority = (
        scenario_impacts.sort_values(["scenario", "priority_rank_scenario"])
        .groupby("scenario", as_index=False)
        .head(15)
    )

    write_df(scenario_impacts, paths.data_processed / "scenario_impacts.csv")
    write_df(summary, paths.data_processed / "scenario_summary.csv")
    write_df(priority, paths.data_processed / "scenario_priority_ranking.csv")
    ...
    return {
        ...
        "scenario_priority_ranking": priority,
    }


if __name__ == "__main__":
    out = run_scenario_engine()
