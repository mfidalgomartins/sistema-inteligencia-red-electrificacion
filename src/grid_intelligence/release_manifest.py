"""Manifiesto de release: hashes y trazabilidad de los artefactos públicos."""
...
from .common import ensure_dirs, get_paths, project_relative
...
ARTIFACTS = {
    ...
    "feeder_priorities": lambda p: p.data_processed / "prioridades_inversion_alimentadores.csv",
    "scenario_summary": lambda p: p.data_processed / "scenario_summary.csv",
    "forecast_calibration": lambda p: p.data_processed / "forecast_calibration_summary.csv",
    "forecast_monitoring": lambda p: p.data_processed / "forecast_monitoring_status.csv",
    "source_contracts": lambda p: p.root / "src/grid_intelligence/resources/source_contracts.json",
}
...
def build_release_manifest() -> dict:
    ...
    if scoring_path.exists():
        ...
        top_zone = (
            scoring.sort_values("investment_priority_score", ascending=False).head(1)["zona_id"].iloc[0]
    ...
    manifest = {
        ...
        "source_mode": validation_summary.get("source_mode", "N/A"),
