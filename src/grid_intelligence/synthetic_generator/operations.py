"""Eventos operativos sintéticos: congestión, interrupciones, flexibilidad y almacenamiento."""
...
def generate_recursos_flexibilidad(zonas_red: pd.DataFrame, seed: int) -> pd.DataFrame:
    ...
    for zona in zonas_red.itertuples(index=False):
        ...
        for tipo in tipos_local:
            ...
            mad = np.clip(
                0.4 + 0.45 * zona.potencial_flexibilidad + 0.12 * zona.densidad_demanda + rng.normal(0, 0.05), 0.1, 0.99
            )
...
def generate_almacenamiento_distribuido(zonas_red: pd.DataFrame, seed: int) -> pd.DataFrame:
    ...
    for zona in zonas_red.itertuples(index=False):
        n_storage = int(
            np.clip(np.round(0.6 + 2.0 * zona.penetracion_generacion_distribuida + rng.normal(0, 0.4)), 1, 3)
        )
        for _ in range(n_storage):
            pot = np.clip(
                0.8
                + 10 * zona.penetracion_generacion_distribuida
                + 6 * zona.potencial_flexibilidad
                + rng.normal(0, 1.4),
                0.4,
                28,
            )
...
def generate_eventos_congestion(
    ...
    flex_cov["coverage"] = (flex_cov["capacidad_flexible_mw"] + 0.82 * flex_cov["capacidad_potencia_mw"]) / np.maximum(
        flex_cov["peak_zona_mw"], 1e-6
    )
    ...
    flagged = df.loc[
        congested_flag,
        ["timestamp", "zona_id", "subestacion_id", "alimentador_id", "tipo_red", "hora", "demanda_mw", "capacidad_mw"],
    ].copy()
    if flagged.empty:
        return pd.DataFrame(
            columns=[
    ...
    flagged["overload_mw"] = np.maximum(flagged["demanda_mw"] - flagged["capacidad_mw"], 0) + 0.28 * np.maximum(
        flagged["demanda_mw"] - 0.95 * flagged["capacidad_mw"], 0
    )
    ...
    grouped["impacto_servicio_flag"] = (
        (grouped["severidad"].isin(["alta", "critica"]))
        | (grouped["energia_afectada_mwh"] > grouped["energia_afectada_mwh"].quantile(0.82))
    ).astype(np.int8)
...
def generate_interrupciones_servicio(
    ...
    risk_sub = (
        ...
        .merge(
            subestaciones[["subestacion_id", "capacidad_mw", "antiguedad_anios", "indice_criticidad"]],
            on="subestacion_id",
            how="left",
        )
    )
    ...
    cong_sub = (
        eventos_congestion.groupby("subestacion_id", as_index=False).size().rename(columns={"size": "n_congestion"})
    )
    risk_sub = risk_sub.merge(cong_sub, on="subestacion_id", how="left").fillna({"n_congestion": 0})

    avg_load_sub = (
        demanda_horaria.groupby("subestacion_id", as_index=False)["demanda_mw"]
        .mean()
        .rename(columns={"demanda_mw": "avg_load_mw"})
    )
    ...
    for row in risk_sub.itertuples(index=False):
        ...
        for _ in range(n_interruptions):
            ...
            if rel_cong and not windows_sub.empty:
                ...
                dur_h = float(
                    np.clip(rng.gamma(shape=2.0 + 2.2 * row.prob_media, scale=0.8), 0.12, min(11.5, w_h + 2.5))
                )
            ...
            sev = (
                "critica"
                if (dur_h >= 5 or ens > 42)
                else "alta"
                if (dur_h >= 2 or ens > 14)
                else "media"
                if dur_h >= 0.7
                else "baja"
            )
            ...
            if rel_cong:
                causa = rng.choice(
                    ["sobrecarga_local", "protecciones", "desconexiones_preventivas"], p=[0.45, 0.33, 0.22]
                )
            else:
                causa = rng.choice(
                    ["fallo_equipo", "evento_climatico", "maniobra", "terceros"], p=[0.34, 0.29, 0.22, 0.15]
                )
...
def generate_intervenciones_operativas(
    ...
    flex_zone = (
        recursos_flexibilidad.groupby("zona_id", as_index=False)["capacidad_flexible_mw"]
        .sum()
        .rename(columns={"capacidad_flexible_mw": "flex_mw"})
    )
    storage_zone = (
        almacenamiento_distribuido.groupby("zona_id", as_index=False)["capacidad_potencia_mw"]
        .sum()
        .rename(columns={"capacidad_potencia_mw": "storage_mw"})
    )
    zone = (
        zonas_red[["zona_id", "potencial_flexibilidad", "criticidad_territorial", "tension_crecimiento_demanda"]]
        .merge(flex_zone, on="zona_id", how="left")
        .merge(storage_zone, on="zona_id", how="left")
        .fillna(0)
    )
        ...
        for tipo in tipos:
            ...
            cap_relief = (
                ...
                * (
                    0.25 * z.flex_mw
                    + 0.35 * z.storage_mw
                    + 8 * z.potencial_flexibilidad
                    + 4 * z.tension_crecimiento_demanda
                )
                * rng.uniform(0.82, 1.25)
            )
            coste = np.clip(
                12_000 + 58_000 * relief_base + 85_000 * z.criticidad_territorial + rng.normal(0, 8_500), 8_000, 520_000
            )
            tiempo = int(
                np.clip(np.round(3 + 28 * relief_base + 24 * z.criticidad_territorial + rng.normal(0, 3)), 2, 180)
            )
            complejidad = np.clip(
                0.25 + 0.45 * relief_base + 0.3 * z.criticidad_territorial + rng.normal(0, 0.06), 0.05, 0.99
            )
...
def generate_inversiones_posibles(
    ...
    sub_cap = (
        subestaciones.groupby("zona_id", as_index=False)["capacidad_mw"]
        .sum()
        .rename(columns={"capacidad_mw": "capacidad_sub_mw"})
    )
    feeders_zone = subestaciones[["subestacion_id", "zona_id"]].merge(alimentadores, on="subestacion_id", how="left")
    cap_feeder = (
        feeders_zone.groupby("zona_id", as_index=False)["capacidad_mw"]
        .sum()
        .rename(columns={"capacidad_mw": "capacidad_feed_mw"})
    )
    ...
    zone = (
        zonas_red[
            [
                ...
                "potencial_flexibilidad",
                "riesgo_climatico",
            ]
        ]
        .merge(sub_cap, on="zona_id", how="left")
        .merge(cap_feeder, on="zona_id", how="left")
    )
    ...
    for z in zone.itertuples(index=False):
        for tipo in tipos:
            ...
            opex = np.clip(
                35_000 + 0.018 * capex + 24_000 * (1 - z.potencial_flexibilidad) + rng.normal(0, 8_000), 9_000, 520_000
            )
            red_risk = np.clip(
                0.12 + 0.72 * risk_factor + 0.2 * z.criticidad_territorial + rng.normal(0, 0.04), 0.05, 0.98
            )
            cap_gain = np.clip(
                cap_factor * (z.capacidad_sub_mw + 0.45 * z.capacidad_feed_mw) * rng.uniform(0.65, 1.35), 1.0, 240
            )
            horizon = int(np.clip(np.round(6 + 54 * cap_factor + 24 * z.riesgo_climatico + rng.normal(0, 4)), 4, 84))
            facilidad = np.clip(
                0.78
                - 0.48 * z.criticidad_territorial
                - 0.18 * z.riesgo_climatico
                + 0.17 * z.potencial_flexibilidad
                + rng.normal(0, 0.05),
                0.05,
                0.98,
            )
            impacto_res = np.clip(
                0.22 + 0.68 * risk_factor + 0.16 * z.riesgo_climatico + rng.normal(0, 0.05), 0.05, 0.99
            )
