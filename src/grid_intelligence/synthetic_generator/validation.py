"""Validaciones internas del dataset sintético generado."""
...
def run_plausibility_checks(tables: dict[str, pd.DataFrame], config: SyntheticDataConfig) -> pd.DataFrame:
    ...
    checks.append(
        {
            ...
            "detalle": "Horas únicas en demanda_horaria",
        }
    )
    ...
    checks.append(
        {
            ...
            "detalle": "Proporción de filas GD con vertido positivo",
        }
    )
    ...
    # Vehículos eléctricos e industrial deben tener peso visible en punta.
    demand_peak = (
        demand[demand["hora_punta_flag"] == 1]
        .groupby("timestamp", as_index=False)["demanda_mw"]
        .sum()["demanda_mw"]
        .mean()
    )
    ev_peak = ev.groupby("timestamp", as_index=False)["demanda_ev_mw"].sum()["demanda_ev_mw"].mean()
    ind_peak = (
        ind.groupby("timestamp", as_index=False)["demanda_industrial_adicional_mw"]
        .sum()["demanda_industrial_adicional_mw"]
        .mean()
    )
    ...
    checks.append(
        {
            ...
            "detalle": "Relación media de vehículos eléctricos + industrial sobre demanda en punta",
        }
    )
...
def write_logic_summary(
    ...
    text = f"""
# Resumen de Lógica del Generador Sintético

## Configuración principal
- Semilla global: {config.seed}
- Periodo: {config.start_timestamp} a {config.end_timestamp}
- Horas únicas simuladas: {demanda["timestamp"].nunique()}
...
## Lógica sintética aplicada
1. Topología coherente jerárquica (zona -> subestación -> alimentador).
2. Estacionalidad diaria, semanal y anual en demanda.
3. Perfiles diferenciales por tipo de zona y tipo de red.
4. Componentes de vehículos eléctricos e industrial correlacionados con crecimiento estructural.
5. Generación distribuida tecnológica (solar/eólica/cogeneración) con autoconsumo y vertido.
6. Congestión modelada sobre utilización, cobertura de flexibilidad y eventos de punta.
7. Interrupciones correlacionadas con envejecimiento de activos y estrés por congestión.
8. Recursos de flexibilidad y almacenamiento con cobertura desigual entre zonas.
9. Catálogos de intervenciones e inversiones con compensaciones técnico-económicas.

## Magnitudes resultantes
- Filas de demanda horaria: {len(demanda):,}
- Filas de generación distribuida: {len(gd):,}
- Eventos de congestión: {len(congestion):,}
- Interrupciones de servicio: {len(interrupciones):,}
- Vertido total estimado (MWh de referencia): {gd["curtailment_estimado_mw"].sum():,.2f}
