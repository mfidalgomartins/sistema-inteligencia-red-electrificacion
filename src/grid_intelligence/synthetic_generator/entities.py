"""Entidades de red: zonas, subestaciones, alimentadores y activos."""
...
def generate_subestaciones(zonas_red: pd.DataFrame, seed: int) -> pd.DataFrame:
    ...
    for zona in zonas_red.itertuples(index=False):
        ...
        total_zone_capacity = (
            110 + 420 * zona.densidad_demanda + 180 * zona.tension_crecimiento_demanda
        ) * rng.uniform(0.88, 1.18)
        ...
        for i in range(n_subs):
            ...
            critic = np.clip(
                0.35
                + 0.28 * zona.criticidad_territorial
                + 0.22 * (1 - redun)
                + 0.18 * (antig / 58)
                + rng.normal(0, 0.05),
                0.05,
...
def generate_alimentadores(subestaciones: pd.DataFrame, zonas_red: pd.DataFrame, seed: int) -> pd.DataFrame:
    ...
    for sub in subestaciones.itertuples(index=False):
        ...
        for i in range(n_feeders):
            ...
            perdidas = np.clip(
                0.028 + 0.0016 * longitud + 0.04 * (1 - zona.densidad_demanda) + rng.normal(0, 0.006), 0.015, 0.18
            )
            expos = np.clip(0.2 + 0.65 * zona.riesgo_climatico + 0.004 * longitud + rng.normal(0, 0.05), 0.05, 0.99)
            base_load = cap * np.clip(
                0.46 + 0.25 * zona.densidad_demanda + 0.12 * zona.tension_crecimiento_demanda + rng.normal(0, 0.05),
                0.25,
                0.93,
            )
            critic = np.clip(
                0.3
                + 0.32 * zona.criticidad_territorial
                + 0.22 * expos
                + 0.14 * (base_load / cap)
                + rng.normal(0, 0.04),
                0.05,
...
def generate_activos_red(
    ...
    for sub in subestaciones.itertuples(index=False):
        for tipo_activo in ["transformador_potencia", "interruptor_principal", "proteccion_digital"]:
            factor_tipo = {"transformador_potencia": 1.0, "interruptor_principal": 0.7, "proteccion_digital": 0.55}[
                tipo_activo
            ]
            ...
            fallo = np.clip(
                0.01 + (100 - salud) / 150 + 0.35 * sub.indice_criticidad * factor_tipo + rng.normal(0, 0.01),
                0.005,
                0.98,
            )
    ...
    feeder_with_zone = alimentadores.merge(
        subestaciones[["subestacion_id", "zona_id", "antiguedad_anios"]], on="subestacion_id", how="left"
    )
    for row in feeder_with_zone.itertuples(index=False):
        ...
        for tipo_activo in ["linea_mt", "reconectador", "sensor_corriente"]:
            ...
            fallo = np.clip(
                0.015
                + (100 - salud) / 135
                + 0.22 * row.criticidad_operativa * factor_tipo
                + 0.08 * zona.riesgo_climatico,
                0.006,
                0.99,
            )
            critic = np.clip(
                0.34 + 0.48 * row.criticidad_operativa + 0.14 * zona.criticidad_territorial + rng.normal(0, 0.05),
                0.05,
                0.99,
            )
