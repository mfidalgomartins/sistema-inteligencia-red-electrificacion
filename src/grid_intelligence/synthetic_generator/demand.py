"""Series de demanda horaria: base, EV e industrial, con estacionalidad controlada."""
...
def create_time_features(config: SyntheticDataConfig) -> pd.DataFrame:
    ...
    factor_estacional = (
        1.0 + 0.10 * np.cos((day_of_year - 15) * 2 * np.pi / 365.25) + 0.07 * np.exp(-(((mes - 7) / 2.0) ** 2))
    )
...
def generate_demanda_ev(
    ...
    for zona in zonas_red.itertuples(index=False):
        base_pen = np.clip(
            0.045 + 0.22 * zona.densidad_demanda + 0.26 * zona.tension_crecimiento_demanda + rng.normal(0, 0.015),
            0.02,
            0.65,
        )
...
def generate_demanda_horaria(
    ...
    ev_zone_hour = (
        demanda_ev.groupby(["zona_id", "timestamp"], as_index=False)["demanda_ev_mw"]
        .sum()
    ...
    ind_zone_hour = (
        demanda_industrial.groupby(["zona_id", "timestamp"], as_index=False)["demanda_industrial_adicional_mw"]
        .sum()
    ...
    feeder_map["base_weight"] = feeder_map["carga_base_esperada"] / feeder_map.groupby("zona_id")[
        "carga_base_esperada"
    ].transform("sum")
    ev_coef = np.where(
    ...
    weather_component = weather_sensitivity * (
        0.045 * np.maximum(0, 18 - temperature) + 0.06 * np.maximum(0, temperature - 27)
    )

    ev_component = (
        ev_zone_hour[zone_idx_expanded, time_idx] * feeder_map["weight_ev"].to_numpy(dtype=np.float32)[feeder_idx]
    )
    ind_component = (
        ind_zone_hour[zone_idx_expanded, time_idx] * feeder_map["weight_ind"].to_numpy(dtype=np.float32)[feeder_idx]
    )
    ...
    reactive = np.clip(
        demand * (0.22 + 0.08 * np.sin((hora - 3) * 2 * np.pi / 24) + rng.normal(0, 0.015, len(demand))), 0.01, None
    )
