"""Generación distribuida sintética y curtailment asociado."""
...
def generate_generacion_distribuida(
    ...
    for zona in zonas_red.itertuples(index=False):
        ...
        cap_cogen = (
            base_capacity * rng.uniform(0.08, 0.2) * (1.1 if zona.tipo_zona in {"industrial", "mixta"} else 0.75)
        )
        ...
        for tech in techs:
            ...
            if tech == "solar_fotovoltaica":
                self_ratio = np.clip(
                    0.42 + 0.12 * zona.densidad_demanda + 0.06 * zona.potencial_flexibilidad, 0.2, 0.78
                )
            elif tech == "eolica_distribuida":
            ...
            autoconsumo = np.minimum(
                generation * self_ratio, zone_demand * np.clip(0.14 + 0.5 * self_ratio, 0.06, 0.85)
            )
            vertido = np.maximum(generation - autoconsumo, 0)

            absorcion_local = zone_demand * np.clip(
                0.02 + 0.08 * zona.potencial_flexibilidad + 0.03 * demand_idx, 0.008, 0.22
            )
            headroom_red = np.clip(
                0.16 + 0.28 * (1 - zona.potencial_flexibilidad) + 0.22 * zona.criticidad_territorial, 0.12, 0.58
            )
