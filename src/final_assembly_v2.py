...
def run_final_assembly_v2() -> dict[str, str]:
    ...
    summary = dedent(
        ...
        - Documentación: feature_dictionary, sql_architecture, sql_metric_definitions, scoring_framework_v2, dashboard_architecture.
        ...
        {pd.DataFrame({'file': processed_files}).to_markdown(index=False)}
        ...
        ## 6) Outputs generados
        - Charts: {len(chart_files)} archivos PNG.
        - Reports: {len(report_files)} archivos en outputs/reports.
        ...
        - Estado global validación: {validation_summary.get('overall_status', 'N/A')}
        - Nivel de confianza: {validation_summary.get('confidence_level', 'N/A')}
        ...
        - validation_status: {smoke.get('validation_status', 'unknown')}
        - publish_state: {smoke.get('publish_state', 'unknown')}
        - warnings: {smoke.get('warnings', '') or 'none'}
        - errors: {smoke.get('errors', '') or 'none'}

        ## Release manifest
        - manifest_version: {release_manifest.get('manifest_version', 'N/A')}
        - top_zone_by_priority: {release_manifest.get('top_zone_by_priority', 'N/A')}
        - dashboard_sha256: {release_manifest.get('artifacts', {}).get('dashboard', {}).get('sha256', 'N/A')}
        """
    ).strip() + "\n"
